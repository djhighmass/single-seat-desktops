...
- **functional/test_customize_foot.sh** -- `wayfire-customize.sh --apply`
  ...
  stub's real argv and where its real stdout/stderr landed.
...
- **functional/test_pkexec_wayland.sh** (needs sudo) -- proves the actual
  mechanism against whichever compositor is live: given the caller's
  real `XDG_RUNTIME_DIR`/`WAYLAND_DISPLAY` passed as arguments, a
...
- **disruptive/test_switch_desktop.sh** (needs sudo, visibly takes over
  ...
  isolation, and the forced-crash-triggers-rollback path.
